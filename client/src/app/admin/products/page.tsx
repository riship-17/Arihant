  ...
  return (
    <div>
      ...
      <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm mb-6 flex items-center gap-4">
        ...
        <input placeholder="Search products..." className="flex-grow bg-transparent outline-none text-sm" />
      </div>
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              <th className="px-6 py-4 text-xs uppercase tracking-widest text-gray-400 font-bold">Product</th>
            ...
            {products.map((p) => (
              <tr key={p._id} className="hover:bg-gray-50/50 transition-colors">
                <td className="px-6 py-4">
                  <div className="font-bold text-brand-secondary text-sm">{p.name}</div>
                  <div className="text-[10px] text-gray-400">Class {p.classRange}</div>
                </td>
                <td className="px-6 py-4 text-sm text-gray-500 capitalize">{p.category}</td>
                <td className="px-6 py-4 text-sm text-gray-500">{p.school?.name}</td>
